                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">Action Types</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 " >
                            <template v-if="$can.view('/action_types/add')">
                                <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/action_types/add`" class="full-width" >
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/action_types"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.Action_TypesTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="action_type" :props="props">
                                                            <span class="table-inlined-edit">{{ props.row.action_type }}</span>
                                                            <inline-edit v-model="props.row.action_type" save-icon="check_circle" cancel-icon="close" :url="`action_types/edit/${ props.row.id }`" field-name="action_type" field-label="Action Type" title="Enter Action Type">
                                                            <api-data-source   api-path="components_data/action_type_option_list"  :query-params="filters" v-slot="req">
                                                                <q-input outlined dense  ref="listctrlaction_type" v-model.trim="props.row.action_type"  label=""  placeholder=""      
                                                                class="" >
                                                                </q-input>
                                                            </api-data-source>
                                                            </inline-edit>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <template v-if="$can.view('action_types/view')">
                                                                                <q-item link clickable v-ripple :to="`/action_types/view/${props.row.id}`">
                                                                                    <q-item-section>
                                                                            ...
                                                                            <template v-if="$can.view('action_types/edit')">
                                                                                <q-item link clickable v-ripple :to="`/action_types/edit/${props.row.id}`">
                                                                                    <q-item-section>
...
<script>
	...
	export default {
		...
		props: {
			...
			routeName : {
				type : String,
				default : 'action_typeslist',
			},
			apiPath : {
				type : String,
				default : 'action_types/index',
			},
			multiCheckbox: {
				type: Boolean,
				default: true,
		...
		computed: {
			...
			records: {
				get: function () {
					return this.$store.getters["action_types/records"];
				},
				set: function (value) {
					this.$store.commit("action_types/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("action_types", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;
