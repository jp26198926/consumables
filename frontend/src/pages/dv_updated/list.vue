                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">Dv Updated</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div >
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/dv_updated"></q-breadcrumbs-el>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <template v-if="$can.view('dv_updated/view')">
                                                                                <q-item link clickable v-ripple :to="`/dv_updated/view/${props.row.id}`">
                                                                                    <q-item-section>
...
<script>
	...
	export default {
		...
		props: {
			...
			routeName : {
				type : String,
				default : 'dv_updatedlist',
			},
			apiPath : {
				type : String,
				default : 'dv_updated/index',
			},
			multiCheckbox: {
				type: Boolean,
				default: false,
		...
		computed: {
			...
			records: {
				get: function () {
					return this.$store.getters["dv_updated/records"];
				},
				set: function (value) {
					this.$store.commit("dv_updated/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
					return this.$store.getters["dv_updated/currentRecord"];
				},
				set: function (value) {
					this.$store.commit("dv_updated/setCurrentRecord", value);
				},
			},
		},
		meta () {
		...
		methods: {
			...mapActions("dv_updated", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;
